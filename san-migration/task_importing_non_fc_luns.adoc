...
summary: FLI では FC LUN のみがサポートされます。ただし、iSCSI LUN をインポートできる回避策があります。他の FLI オンライン 7-Mode からONTAPへのワークフローとは異なり、iSCSI LUN を FC LUN としてインポートするため、中断ウィンドウはこのワークフロー全体に及びます。 
---
= FC 以外の LUN をインポートする
:allow-uri-read: 
:icons: font
...
[role="lead"]
FLI では FC LUN のみがサポートされます。ただし、iSCSI LUN をインポートできる回避策があります。他の FLI オンライン 7-Mode からONTAPへのワークフローとは異なり、iSCSI LUN を FC LUN としてインポートするため、中断ウィンドウはこのワークフロー全体に及びます。
