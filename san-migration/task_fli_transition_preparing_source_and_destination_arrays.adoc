...
summary: FLI の 7-Mode から ONTAP への移行の準備として、ホストとソース LUN のパスおよびその他の詳細を確認します。 
---
= ソースアレイとデスティネーションアレイの移行の準備
:icons: font
:imagesdir: ../media/


[role="lead"]
FLI の 7-Mode から ONTAP への移行の準備として、ホストとソース LUN のパスおよびその他の詳細を確認します。

.手順
. ONTAP で、「 advanced 」権限レベルに変更します。
+
[listing]
----
cluster::> set adv
...
----
. 表示されたストレージエラーの詳細を確認します。エラーによっては、処理を続行する前に対処が必要なものもありただし、次の例に示すエラーは無視しても問題ありません。 "This device is an ONTAP(R) LUN".
+
[NOTE]
====
「 This device is an ONTAP\ （ R ） LUN. 」というエラーメッセージが表示されるのは、 FLI が ONTAP テクノロジに依存していて、 FlexArray または ONTAP ターゲットの仮想化をサポートしていないためです。FLI では LUN をインポートできますが、 FlexArray では仮想化はサポートされていません。
...
Warning: Configuration errors were detected.  Use 'storage errors show' for detailed information.
----
. 7-Mode ストレージからマッピングされている LUN を表示します。ディスクのプロパティとパスを確認します。
+
[listing]
----
...
----
. ターゲット LUN を作成します。lun create コマンドは、パーティションオフセットに基づいてサイズとアライメントを検出し、 foreign-disk 引数に従って LUN を作成します
+
[listing]
----
...
----
. テスト用 LUN をテスト用 igroup にマッピングします。
+
[listing]
----
